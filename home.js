    ...
    $.get(
        ...
        function (data, status) {
            ...
            if (data.ConversionType.type === "divide") {
                getHistoricalPriceDataAlt(
                    coinNumerator, coinDenominator, unixEntered
                )
            } else {
                for (var i = 1999; i >= 1; i--) {
                    ...
                    var dayOfWeek = new Date((data.Data[i].time + 86400) * 1000)
                        .toString()
                        .substr(0, 3);
...
function getHistoricalPriceDataAlt(coinNumerator, coinDenominator, unixEntered) {
    $.get(
        "https://min-api.cryptocompare.com/data/histoday?fsym=" +
        coinNumerator +
        "&tsym=" +
        coinDenominator +
        "&limit=2000&aggregate=1&toTs=" +
        unixEntered + "&e=" + exchange + "&tryConversion=false",
        function (data, status) {
            convert = 1;
            var historicalPrice = [];
            var historicalOpen = [];
...
function getRecentEqualPrices(
    ...
    for (var i = 2000; i >= 0; i--) {
        if (currentPrice >= data.Data[i].low && currentPrice <= data.Data[i].high) {
            var dateSamePrice = new Date((data.Data[i].time + 86400) * 1000);
...
function getCurrentPriceData(coinNumerator, coinDenominator, unixEntered) {
    var currentData = $.get(
        ...
        coinNumerator +
        "&tsyms=" + coinDenominator + "&e=" + exchange,
        function (data, status) {
            ...
            allTimeHighPrice = 0;
                for (var i = 0; i <= 1999; i++) {
                    if (data.Data[i].high > allTimeHighPrice && data.Data[i].high <= (5 * data.Data[i].close)) {
                        var allTimeHighDateUnix = data.Data[i].time + 86400;
...
function plotWeekdayChange(
    ...
    for (var i = 0; i <= 6; i++) {
        ...
        console.log(weeklyChangeMedian);
        console.log(weeklyChangeAverage)
    ...
    $("#radio2").click(function () {
        if (dataSetPlotted.length === 1) {
            ...
            console.log(dataSetPlotted.length)
...
function getHistoricalHourlyData(coinNumerator, coinDenominator, unixEntered) {
  ...
  $.get(
"https://min-api.cryptocompare.com/data/histohour?fsym=" + coinNumerator + "&tsym=" + coinDenominator + "&limit=2000&aggregate=1&e=" + exchange,
      function (data, status) {
          if (data.ConversionType.type === "divide") {
              ...
              $.each(hourlyHour, function (ind, itm) {
                ...
                //  console.log(groups3[itm].hourValues);
              });
...
function getHistoricalHourlyDataAlt(coinNumerator, coinDenominator, unixEntered) {
    ...
    var hourlyPriceChange = [];
    $.get(
        "https://min-api.cryptocompare.com/data/histohour?fsym=" + coinNumerator + "&tsym=" + coinDenominator + "&limit=2000&aggregate=1&e=" + exchange + "&tryConversion=false",
        function (data, status) {
            for (i = (2001 - (filterDays * 24)); i <= data.Data.length - 1; i++) {
                var hourlyDateInt = new Date(data.Data[i].time * 1000);
                var hourlyHourInt = hourlyDateInt.toString().slice(16, 18);
...
function plotHourlyData(
  hourlyLongDate, hourlyDate, hourlyHour, hourlyPrice, errorBars, hourlyPriceChange, coinNumerator, coinDenominator, hourlyChangeMedian
) {
